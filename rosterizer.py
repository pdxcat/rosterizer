...
zombie_errors = []
for name, username in zombies:
    roster_text += roster_line.format(name=name, username=username)
    try:
        chronicle.wiki_page.create(project_id=project, title=username, text=zombie_text.format(name=name, username=username), parent_title=roster_title, comments="Autogenerated by Rosterizer")
    except ValidationError as e:
        print("Couldn't create page for {0} ({1}). Skipping.".format(name, str(e)))
        zombie_errors.append((name, username))
    else:
...
print
print("Done! {host}/projects/{project}/wiki/{roster_title}".format(host=secret['host'], project=project, roster_title=roster_title.replace(" ", "_")))
